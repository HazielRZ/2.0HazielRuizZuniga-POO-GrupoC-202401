...
public class GestorEmpleados {
    protected static ArrayList<Empleado> listaEmpleados;
    ...
    public static Empleado[] getEmpleadosPorSucursal(Sucursal sucursal) {
        ArrayList<Empleado> empleadosEnSucursal = new ArrayList<>();

        // Iterar sobre la lista de clientes y agregar aquellos que pertenecen a la sucursal actual
        for (Empleado empleado : listaEmpleados) {
            if (empleado.getSucursalPerteneciente().equals(Menu.sucursalActual)) {
               empleadosEnSucursal.add(empleado);
            }
        }

        // Convertir el ArrayList a un array de Cliente y devolverlo
        return empleadosEnSucursal.toArray(new Empleado[0]);
    }
    ...
    public void modificarEmpleado(int idEmpleado, String nombre, String apellidos, int añoNacimiento, String ciudad,
                                  String estado, String direccion, Sucursal sucursalPerteneciente, double salario,
